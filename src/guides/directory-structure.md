...
Cache file directory generated by WinJS during builds, storing `babel` cache, bundler cache, etc.

::: tip Tip

If a build fails, you can delete the `.cache` directory, which will be regenerated on the next build. You can also directly execute `win cache clean` to clear it.
